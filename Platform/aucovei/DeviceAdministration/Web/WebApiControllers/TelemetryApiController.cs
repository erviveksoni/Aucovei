...
namespace Microsoft.Azure.Devices.Applications.RemoteMonitoring.DeviceAdmin.Web.WebApiControllers
{
    ...
    public class TelemetryApiController : WebApiControllerBase
    {
        ...
        [HttpGet]
        [Route("wavepointforroute")]
        [WebApiRequirePermission(Permission.ViewTelemetry)]
        public async Task<HttpResponseMessage> GetWayPointForRoute(string routeId, string pointId)
        {
            Func<Task<RouteWavepointModel>> routeWavepoint =
                async () =>
                {
                    return await _deviceTelemetryLogic.LoadWavepointForRoute(routeId, pointId);
                };
            this.mediaType = "application/json";
            HttpResponseMessage response = await GetServiceResponseAsync<RouteWavepointModel>(routeWavepoint, false);
            return response;
        }

        [HttpGet]
        [Route("wavepointlistforroute")]
        [WebApiRequirePermission(Permission.ViewTelemetry)]
        public async Task<HttpResponseMessage> GetWayPointListForRoute(string routeId)
        {
            Func<Task<RouteWavepointListModel>> routeWavepoints =
                async () =>
                {
                    return await _deviceTelemetryLogic.LoadWavepointsListForRoute(routeId);
                };
            this.mediaType = "application/json";
            HttpResponseMessage response = await GetServiceResponseAsync<RouteWavepointListModel>(routeWavepoints, false);
            return response;
        }
