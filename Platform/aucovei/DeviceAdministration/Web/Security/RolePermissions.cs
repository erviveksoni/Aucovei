...
namespace Microsoft.Azure.Devices.Applications.RemoteMonitoring.DeviceAdmin.Web.Security
{
    ...
    public class RolePermissions
    {
        ...
        public bool HasPermission(Permission permission, HttpContextBase httpContext)
        {
            ...
            var userClaims = identity.Claims.ToList().FindAll(i => i.Type.Equals("http://schemas.microsoft.com/ws/2008/06/identity/claims/role", System.StringComparison.OrdinalIgnoreCase));

            if (userClaims != null)
            {
                var userroles = userClaims.Select(i => i.Value);
                foreach (var role in _allRoles)
                {
                    ...
                    //    return true;
                    //}
                    if (userroles.Contains(role) && rolesRequired.Contains(role))
                    {
                        return true;
                    }
                }
