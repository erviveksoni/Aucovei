...
namespace Microsoft.Azure.Devices.Applications.RemoteMonitoring.DeviceAdmin.Infrastructure.BusinessLogic
{
    public class DeviceLogic : IDeviceLogic
    {
        ...
        {
            return await this._deviceRegistryListRepository.GetDeviceList(q);
        }
        ...
        {
            return await this._deviceRegistryCrudRepository.GetDeviceAsync(deviceId);
        }
        ...
        private async Task<dynamic> AddDeviceToRepositoriesAsync(dynamic device, SecurityKeys securityKeys)
        {
            ...
            await this._iotHubRepository.AddDeviceAsync(device, securityKeys);

            try
            {
                registryRepositoryDevice = await this._deviceRegistryCrudRepository.AddDeviceAsync(device);
            }
            catch (Exception ex)
            {
            ...
            if (capturedException == null && isSimulatedAsBool)
            {
                try
                {
                    await this._virtualDeviceStorage.AddOrUpdateDeviceAsync(new InitialDeviceConfig()
                    {
                        DeviceId = DeviceSchemaHelper.GetDeviceID(device),
                        HostName = this._configProvider.GetConfigurationSettingValue("iotHub.HostName"),
            ...
            if (capturedException != null)
            {
                ...
                // in some scenarios.
                await this._iotHubRepository.TryRemoveDeviceAsync(DeviceSchemaHelper.GetDeviceID(device));
        ...
        public async Task RemoveDeviceAsync(string deviceId)
        {
            ...
            await this._iotHubRepository.RemoveDeviceAsync(deviceId);

            try
            {
                await this._deviceRegistryCrudRepository.RemoveDeviceAsync(deviceId);
            }
            catch (Exception ex)
            {
            ...
            if (capturedException == null)
            {
                try
                {
                    await this._virtualDeviceStorage.RemoveDeviceAsync(deviceId);
                }
                catch (Exception ex)
                {
                ...
                await this._deviceRulesLogic.RemoveAllRulesForDeviceAsync(deviceId);
            }
            else
            {
                ...
                await this._iotHubRepository.TryAddDeviceAsync(iotHubDevice);
                capturedException.Throw();
            }
        }
        ...
        {
            return await this._deviceRegistryCrudRepository.UpdateDeviceAsync(device);
        }
        ...
        public async Task<dynamic> UpdateDeviceFromDeviceInfoPacketAsync(dynamic device)
        {
            ...
            return await this._deviceRegistryCrudRepository.UpdateDeviceAsync(existingDevice);
        }
        ...
        {
            return await this._iotHubRepository.GetDeviceKeysAsync(deviceId);
        }
        ...
        public async Task<dynamic> UpdateDeviceEnabledStatusAsync(string deviceId, bool isEnabled)
        {
            ...
            // if an exception happens at this point pass it up the stack to handle it
            await this._iotHubRepository.UpdateDeviceEnabledStatusAsync(deviceId, isEnabled);

            try
            {
                registryRepositoryDevice =
                    await this._deviceRegistryCrudRepository.UpdateDeviceEnabledStatusAsync(deviceId, isEnabled);
            }
            catch (Exception ex)
            {
                // grab the exception so we can attempt an async removal of the device from the IotHub
        ...
        {
            dynamic device = await this.GetDeviceAsync(deviceId);
